...
Redaktionella yrkessidor får använda verifierade positiva `occupation-name`-filter när en bred JobTech-grupp blandar flera tydligt skilda roller. Alla kuratorsinriktningar samlas på den gemensamma landningssidan för Kurator, som omfattar benämningarna Kurator, Skolkurator och Hälso- och sjukvårdskurator men inte Familjebehandlare. Tidigare landningssideadresser för Skolkurator omdirigeras permanent till motsvarande Kurator-sida; en separat yrkesguide om skolkurator kan fortfarande finnas eftersom den fyller ett redaktionellt syfte.

JobSearch kan innehålla flera källposter för samma ansökningstillfälle. Jobblistor och jobbsitemap deduplicerar därför konservativt före mappning när posterna har samma normaliserade titel, arbetsgivare, sista ansökningsdag och externa ansöknings-URL. I jobblistor, där annonstext och ort redan hämtas, kan även poster utan direkt ansöknings-URL dedupliceras när både ort och annonstext är identiska. Den högsta källidentiteten behålls deterministiskt. Exakta titeln `Familjehem` filtreras lokalt eftersom den avser ett uppdrag och inte en anställning; närliggande anställningar som `Familjehemssekreterare` behålls.

Söklistor hämtar totalantalet genom ett separat minimalt och gemensamt cachat JobSearch-svar. Varje vy hämtar och bearbetar endast sin ordinarie resultatsida: 6 poster på startsidan, 20 i jobblistan och 4 för relaterade jobb. Lokalt bortvalda eller deduplicerade poster fylls inte på genom överhämtning, eftersom låg server-CPU prioriteras framför exakt lika många kort på varje sida. Därmed delar jobblistans paginerade sidor samma totalsnapshot utan att startsidan eller relaterade jobb hämtar extra annonsdata. Startsidan har dessutom en separat ISR-cache och visar därför totalen uttryckligen som ett ungefärligt värde avrundat till närmaste hundratal.
